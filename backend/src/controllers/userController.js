...
export const getFollowing = async (req, res) => {
  ...
  const { data, error } = await db
    ...
    .eq("user_id", userId);
  if (error) return handleError(res, error);
  return res.json(data.map((item) => item.artist));
};
...
export const setProfilePhoto = async (req, res) => {
  ...
  if (!url)
    return res.status(400).json({ error: "Url da imagem é obrigatório." });
...
export const changePassword = async (req, res) => {
  ...
  if (!newPassword)
    return res.status(400).json({ error: "Digite uma nova senha." });
